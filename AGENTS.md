...
- RAW の埋め込み JPEG プレビュー抽出は、現時点では RAW コンテナを解析せず JPEG SOI/EOI marker を走査して最大候補を選ぶ best-effort fallback として扱う。中長期では LibRaw thumbnail extraction の WASM 化、サーバー側 LibRaw/ExifTool/exiv2 fallback、DNG の TIFF/IFD preview 解析、実機 RAW サンプル回帰テストを検討する。詳細は `spec/index.md` と `spec/architecture.md` を参照する。
