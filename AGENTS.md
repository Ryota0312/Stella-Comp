...
- 現在の最小縦断は preview JPEG first。Web UI は RAW の D&D 直後には重い RAW 現像をせず、`libraw-wasm` thumbnail または fallback 抽出した埋め込み JPEG、あるいはブラウザで軽量生成した preview JPEG を `POST /api/preview-uploads` に送る。その後 `POST /api/preview-alignments` で Rust worker の `EstimateTransforms` を呼ぶ非同期ジョブを作成し、`GET /api/preview-alignments/:alignmentJobID` でpreview座標系の変換行列を受け取り、ブラウザ Canvas でpreview JPEGを加算平均合成する。標準の変換モデルは `homography` とする。この preview upload/preview 合成は D&D 後に自動実行してよい。結果確認 UI では合成結果と基準 preview JPEG の切替/左右比較、カーソル位置のピクセル等倍確認をクライアント側で行う。ユーザーが preview 結果を確認して本画像合成ステップへ進んだ時点で、`libraw-wasm` による RAW 現像と元画像合成を開始する。本画像合成ステップ内の実行ボタンは再実行や将来のオプション変更後の実行用として残す。RAW 現像と元画像合成中は進捗を表示する。`POST /api/jobs` は Rust worker の `AlignAndAverage` によるサーバー側preview合成の比較・フォールバック用として残す。
- RAW の軽量 preview 抽出は、公開版 `libraw-wasm@1.5.0` の `thumbnailData()` による JPEG thumbnail 抽出を第一候補にする。LibRaw thumbnail が利用できない場合は、RAW コンテナを解析せず JPEG SOI/EOI marker を走査して最大候補を選ぶ best-effort fallback に切り替える。この marker scan はあくまで互換性のための fallback として扱う。中長期ではサーバー側 LibRaw/ExifTool/exiv2 fallback、DNG の TIFF/IFD preview 解析、実機 RAW サンプル回帰テストを検討する。詳細は `spec/index.md` と `spec/architecture.md` を参照する。
...
- Web フロントエンドの feature は業務ドメイン単位で分ける。現在の星景写真合成ワークスペースは `apps/web/src/features/stacking` を正とし、アップロード、プレビュー合成、本画像合成は同 feature 内のフェーズとして扱う。UI 固有 CSS は `app/globals.css` に集約せず、feature 近くの CSS Modules に置く。
- `apps/web/src/features/stacking` のワークフロー状態は feature-local な Zustand store と provider/context で扱い、`StackingWorkspace` に props 配線を集中させない。単一コンポーネント内で完結する表示状態は local state のままにする。
- 最終結果フェーズの基準/合成比較と等倍確認では、preview JPEG ではなく、合成結果と同じ RAW 現像または元画像デコード経路で生成した基準フレーム PNG を比較対象にする。
- 変換行列推定に失敗して `ImageTransform.estimated=false` になった非基準フレームは、preview 合成と本画像合成の両方で除外する。通常 UI には内部 warning code/message を出さず、ローカライズした要約警告と実際に使用するフレーム数を表示する。内部 warning code/message は staging debug のみで表示する。
