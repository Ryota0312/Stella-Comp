...
- 現在の埋め込み JPEG 抽出は、RAW 形式ごとの仕様やメーカー固有メタデータを解釈しない。短期的には best-effort fallback として明示し、失敗時は `RAW pending` とする。
- 中期では LibRaw の thumbnail extraction 相当をブラウザ WASM から呼び出せるよう、`libraw-wasm` の拡張、fork、別 wrapper を検討する。
