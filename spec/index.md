...
preview JPEG の位置合わせは、標準では星検出ベースの特徴点を使い、短時間の星景フレームに合わせて回転・平行移動・等方スケールの部分アフィン変換を推定する。旧方式として AKAZE 特徴点ベースの推定も残し、Web UI の位置合わせ方式で `stars` / `akaze` を選択できる。API 互換性のため、方式未指定または未知値は `akaze` として扱う。MVP では、RANSAC で妥当な変換を推定できないフレームは `TRANSFORM_ESTIMATE_FAILED` warning を付けて identity transform を返し、クライアント側合成全体は可能な限り完了させる。画面端の流れが残る場合は、homography やメッシュワープなどの非グローバルアフィン方式を後続で検討する。
