...
preview JPEG の位置合わせは、標準では星検出ベースの特徴点を使い、短時間の星景フレームに合わせて回転・平行移動・等方スケールの部分アフィン変換を推定する。旧方式として AKAZE 特徴点ベースの推定も残し、Web UI の位置合わせ方式で `stars` / `akaze` をプレビュー合成開始前に選択できる。プレビュー結果画面と本画像合成画面では選択済み方式の表示に留め、途中変更する場合はアップロード画面へ戻って preview alignment を再実行する。API 互換性のため、方式未指定または未知値は `akaze` として扱う。MVP では、RANSAC で妥当な変換を推定できないフレームは `TRANSFORM_ESTIMATE_FAILED` warning を付けて identity transform を返し、クライアント側合成全体は可能な限り完了させる。画面端の流れが残る場合は、homography やメッシュワープなどの非グローバルアフィン方式を後続で検討する。
...
- RAW 現像パラメータの扱い
- プレビュー画像の生成方式と最大サイズ
- 位置合わせの変換モデル（affine / homography / 将来の local warp）と品質指標を Protocol Buffers レスポンスへ正式に含めるか、当面は worker 内部ログ・CLI・staging debug に限定するか
