...
preview JPEG の位置合わせは、標準では星検出ベースの特徴点を使い、短時間の星景フレームに合わせて回転・平行移動・等方スケールの部分アフィン変換を推定する。旧方式として AKAZE 特徴点ベースの推定も残し、Web UI の位置合わせ方式で検出方式 `stars` / `akaze` と変換モデル `affine` / `homography` をプレビュー合成開始前に選択できる。標準は `stars + affine` とし、`homography` は実験的な高精度候補として扱う。homography は RANSAC 後にスケール範囲に加えて、プレビュー画像全体に対する射影成分と四隅の分母変化を制限し、少数の誤対応や過大な射影変形で画面端だけが大きくずれる行列は失敗扱いにする。プレビュー結果画面と本画像合成画面では選択済み方式の表示に留め、途中変更する場合はアップロード画面へ戻って preview alignment を再実行する。API 互換性のため、検出方式未指定または未知値は `akaze`、変換モデル未指定または未知値は `affine` として扱う。MVP では、RANSAC で妥当な変換を推定できないフレームは `TRANSFORM_ESTIMATE_FAILED` warning を付けて identity transform を返し、クライアント側合成全体は可能な限り完了させる。画面端の流れが残る場合は、メッシュワープなどの非グローバル変換方式を後続で検討する。

位置合わせアルゴリズムの改善は `spec/alignment-roadmap.md` に従う。直近では検出手法の選択をプレビュー生成前へ移動し、現在の `stars + affine` を標準として維持しながら、アフィン安定化、ホモグラフィ追加、アフィン/ホモグラフィ比較評価、対応星残差可視化、局所ワープの順に進める。通常 UI は詳細なデバッグ値を増やさず、必要な比較値は CLI example または staging debug で確認できるようにする。
