...
初回アクセス時には、`Stella Comp` が開発中のベータ版であることを知らせるモーダルを表示する。告知では、機能、処理結果、制限、画面構成が今後予告なく変更される可能性があることを明示する。ユーザーが「今後表示しない」をチェックして確認した場合のみ、ブラウザの localStorage に表示抑制状態を保存し、以後のアクセスでは同じ告知を表示しない。チェックせずに閉じた場合は次回アクセス時も再表示する。
...
- Redis 互換キュー基盤は Valkey を標準候補にする。Compose / deploy 環境では Valkey Streams と job state hash を使い、preview 変換行列推定と fallback 合成の待機 queue、並列数制限、pending job 回収を扱う。`STELLA_COMP_QUEUE_URL` が未設定のローカル開発ではプロセス内 store / queue に fallback する。
...
初期の Web 実装では、RAW ファイルも D&D の入力として受け付ける。ただし、ブラウザ標準 API では CR2/CR3 を直接デコードできる前提にしない。D&D 直後は重い RAW 現像を開始せず、まず `libraw-wasm` の `thumbnailData()` で RAW 内の JPEG thumbnail を抽出して軽量 preview JPEG 生成に使う。LibRaw thumbnail が利用できない場合は、既存 Web Worker で JPEG SOI/EOI marker を走査して最大の JPEG 候補を選ぶ best-effort fallback を使う。preview JPEG のアップロード、位置合わせ推定、preview 合成結果生成は D&D 後に自動実行する。ユーザーが preview 合成結果を確認して本画像合成ステップへ進んだ時点で、`libraw-wasm` を使って RAW 本体をブラウザ側 WebAssembly/Worker で現像し、preview で推定済みの変換行列を元画像座標系へ補正して合成する。RAW 現像と元画像合成中は、処理中フレーム数ベースの進捗を表示する。本画像合成ステップ内の実行ボタンは、再実行や将来の本画像合成オプション変更後の実行操作として残す。

初期処理フロー:

1. ブラウザで元画像を選択する。
2. ブラウザで軽量プレビュー画像を生成する。RAW は `libraw-wasm` の thumbnail 抽出を優先し、失敗時だけ best-effort の埋め込み JPEG marker scan に fallback する。この段階では `libraw-wasm` の本現像は実行しない。
3. Go API に軽量プレビュー画像を自動アップロードする。
4. Rust worker が軽量プレビュー画像からホモグラフィ変換行列を自動推定する。
...
preview JPEG の位置合わせは、標準では星検出ベースの特徴点を使い、短時間の星景フレームに合わせてホモグラフィ変換を推定する。旧方式として AKAZE 特徴点ベースの推定も残し、Web UI の位置合わせ方式で検出方式 `stars` / `akaze` と変換モデル `homography` / `affine` をプレビュー合成開始前に選択できる。標準は `stars + homography` とし、`affine` は互換・比較用として扱う。homography は RANSAC 後にスケール範囲に加えて、プレビュー画像全体に対する射影成分と四隅の分母変化を制限し、少数の誤対応や過大な射影変形で画面端だけが大きくずれる行列は失敗扱いにする。プレビュー結果画面と本画像合成画面では選択済み方式の表示に留め、途中変更する場合はアップロード画面へ戻って preview alignment を再実行する。API 互換性のため、検出方式未指定または未知値は `akaze`、変換モデル未指定または未知値は `homography` として扱う。MVP では、RANSAC で妥当な変換を推定できないフレームは `TRANSFORM_ESTIMATE_FAILED` warning を付けて identity transform を返し、クライアント側合成全体は可能な限り完了させる。画面端の流れが残る場合は、メッシュワープなどの非グローバル変換方式を後続で検討する。

位置合わせアルゴリズムの改善は `spec/alignment-roadmap.md` に従う。現在は `stars + homography` を標準とし、アフィンは互換・比較用として残す。通常 UI は詳細なデバッグ値を増やさず、必要な比較値は CLI example または staging debug で確認できるようにする。
...
- 現在の RAW preview 抽出は、公開版 `libraw-wasm@1.5.0` の thumbnail extraction を第一候補にし、JPEG thumbnail が取れない場合だけ RAW 形式ごとの仕様やメーカー固有メタデータを解釈しない SOI/EOI marker scan に fallback する。この marker scan はあくまで best-effort fallback であり、どちらも失敗した場合は `RAW pending` とする。
- 堅牢性を優先する fallback として、Go API または Rust worker 側で LibRaw / ExifTool / exiv2 系ライブラリを使って preview を抽出する経路を検討する。
- DNG は TIFF/IFD 系として thumbnail/preview IFD を読む実装を先に追加できる可能性がある。メーカー独自 RAW は LibRaw 系に寄せる。
- CR2/CR3/DNG/NEF/ARW/RAF/ORF/RW2 の実機サンプルを使った回帰テストを整備し、preview 抽出成功率、LibRaw 現像成功率、preview と本画像の座標対応を確認する。
...
- `POST /api/preview-alignments`
  - JSON の `sessionId`、`baseImageIndex`、`alignmentMethod`、`transformModel` を受け取り、preview upload セッション内のファイルを名前順で Rust worker に渡す非同期ジョブを作成する。
  - レスポンスは `202 Accepted` と `alignmentJobId`、`queued` 状態のジョブ情報。
- `GET /api/preview-alignments/:alignmentJobID`
  - Valkey またはローカル開発用プロセス内 store で管理している `queued` / `running` / `completed` / `failed` の状態を返す。
  - `completed` の場合は各画像を基準preview座標系へ写す `transforms[]` と `warnings[]` を返す。`affine` では 2x3 行列、`homography` では 3x3 行列を使う。
- `POST /api/jobs`
...
Web UI は preview JPEG の準備完了後、自動で preview JPEG をアップロードし、`POST /api/preview-alignments` を呼び出して変換行列推定ジョブを作成し、`GET /api/preview-alignments/:alignmentJobID` を polling して完了後に変換行列を取得し、ブラウザ側で preview JPEG をスタックする。preview 合成結果は Blob URL として画面プレビュー、別タブ表示、PNG ダウンロードリンクに使うため、通常フローではサーバーに合成 PNG を保存しない。RAW 現像と元画像合成の結果は、画面表示用 PNG preview と、ユーザーが選択した TIFF / PNG / JPEG のダウンロードリンクに分ける。warning が返った場合、通常 UI にはローカライズしたユーザー向け要約だけを表示し、`TRANSFORM_ESTIMATE_FAILED` などの warning code と内部 message は staging debug のみに表示する。RAW 現像と元画像合成はユーザーが preview 結果を確認した後の明示操作でのみ開始する。`POST /api/jobs` と `GET /api/jobs/:jobID/result` はサーバー側合成の比較・フォールバック用として残す。
...
- ジョブ履歴・監査方式: 現在の Valkey job state に加えて、監査や長期履歴が必要になった段階で PostgreSQL などの永続 DB を併用するか
- 画像保存先: ローカルファイルシステムから開始するか、S3 互換ストレージを前提にするか
- RAW 現像パラメータの扱い
- プレビュー画像の生成方式と最大サイズ
- 位置合わせの品質指標を Protocol Buffers レスポンスへ正式に含めるか、当面は worker 内部ログ・CLI・staging debug に限定するか
