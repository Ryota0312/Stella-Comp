...
通常のユーザー向け UI は星景写真家が判断に使う情報を優先し、フレーム数、preview 生成状況、位置合わせ/合成状況、本画像処理進捗、書き出し形式、警告だけを主表示にする。preview payload、圧縮率、アップロード件数、alignment job ID、内部ステータス、warning code などのデバッグ情報は `NEXT_PUBLIC_DEPLOY_STAGE=staging` または `NEXT_PUBLIC_APP_ENV=staging` の場合のみ、実行パネル下部の staging debug 枠に表示する。
